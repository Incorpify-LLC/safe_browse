...
{
    client.BaseAddress = new Uri(services.GetRequiredService<IOptions<AgentOptions>>().Value.ApiBaseUrl);
    client.Timeout = TimeSpan.FromSeconds(15);
});
builder.Services.AddHttpClient("doh", (services, client) =>
{
    client.BaseAddress = new Uri(services.GetRequiredService<IOptions<AgentOptions>>().Value.UpstreamDohUrl);
